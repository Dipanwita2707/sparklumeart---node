...
router.post('/orders/:id/status', ensureAuthenticated, ensureAdmin, async (req, res) => {
    try {
        ...
        // Update seller performance metrics when order is delivered
        if (status === 'delivered' && oldStatus !== 'delivered') {
            ...
            for (const item of order.items) {
                if (item.seller) {
                    if (!sellerItems[item.seller._id]) {
                        sellerItems[item.seller._id] = {
                            items: [],
                            totalAmount: 0
                        };
                    }
                    
                    sellerItems[item.seller._id].items.push(item);
                    sellerItems[item.seller._id].totalAmount += (item.price * item.quantity);
                }
            }
            
            // Update performance metrics for each seller
            for (const sellerId in sellerItems) {
                ...
                performance.metrics.totalRevenue += sellerItems[sellerId].totalAmount;
                ...
                console.log(`Updated performance metrics for seller ${sellerId}`);
            }
        }
...
router.post('/leads/:id/assign', ensureAuthenticated, ensureAdmin, async (req, res) => {
    try {
        ...
        if (oldSellerId !== (req.body.sellerId || null)) {
            let newSellerInfo = null;
            
            if (req.body.sellerId) {
                try {
                    newSellerInfo = await User.findById(req.body.sellerId).select('name');
                } catch (err) {
...
router.get('/email-campaigns/:id', ensureAuthenticated, ensureAdmin, async (req, res) => {
    // Special handling for 'new' to avoid the route conflict
...
// Lead Analysis
router.get('/leads/:id/analysis', ensureAuthenticated, ensureAdmin, async (req, res) => {
    try {
        const lead = await Lead.findById(req.params.id);
        if (!lead) {
        ...
        const analysisData = {
            engagementScore: lead.aiScore || 0,
            conversionLikelihood: lead.conversionProbability || 0,
            insights: `This lead shows ${lead.aiScore > 70 ? 'strong' : lead.aiScore > 50 ? 'moderate' : 'low'} engagement based on their activity patterns. The lead's conversion probability is ${lead.conversionProbability > 70 ? 'high' : lead.conversionProbability > 50 ? 'moderate' : 'low'}.`,
            recommendations: [
                {
                    text: 'Schedule a follow-up call to discuss their specific needs',
                    priority: lead.conversionProbability > 60 ? 'high' : 'medium'
                },
                {
                    text: 'Send personalized product recommendations based on their browsing history',
                    priority: lead.aiScore > 50 ? 'high' : 'low'
                },
                {
                    text: 'Offer a discount code to incentivize conversion',
                    priority: lead.conversionProbability < 50 ? 'high' : 'medium'
                },
                {
                    text: 'Share case studies relevant to their interests',
                    priority: 'medium'
                }
            ]
        };
        ...
        res.status(500).json({ error: 'Error fetching lead analysis' });
    }
});
